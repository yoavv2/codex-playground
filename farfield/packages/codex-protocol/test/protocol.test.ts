...
describe("codex-protocol schemas", () => {
  ...
  it("parses thread state requests for command and file approvals", () => {
    const parsed = parseThreadConversationState({
      id: "thread-approval",
      turns: [],
      requests: [
    ...
    expect(parsed.requests[1]?.method).toBe("item/fileChange/requestApproval");
  });

  it("parses thread request response payload for command approval decision", () => {
    const parsed = parseThreadRequestResponsePayload({
      decision: "accept"
    });

    expect(parsed).toEqual({ decision: "accept" });
  });
